...
def simulate_alpha(alpha = 1.2):
    ...
    print (first_schedl, second_schedl, third_schedl)
    return (first_schedl + second_schedl + third_schedl)
...
def main():
    ...
    # previous iterations' total scheduled tasks
    prev_total_sched = 0

    while True:
        cur_total_sched = simulate_alpha(start_alpha)
        print ("Current total scheduled tasks are: ", cur_total_sched, start_alpha)
        if cur_total_sched <= prev_total_sched:
            saturation += 1
            if saturation >= 10:
                break
        else:
        ...
        prev_total_sched = cur_total_sched

    print ("Optimized Alpha is : ", start_alpha)
